...
namespace ET
{
    public class EntitySystem
    {
        ...
        {
            Queue<EntityRef<Entity>> queue = this.queues[InstanceQueueIndex.LogicUpdate];
            int count = queue.Count;
            while (count-- > 0)
            {
                Entity component = queue.Dequeue();
                ...
                if (component is not ILogicUpdate)
                {
                    continue;
                }
                ...
                try
                {
                    List<SystemObject> iUpdateSystems = EntitySystemSingleton.Instance.TypeSystems.GetSystems(component.GetType(), typeof (ILogicUpdateSystem));
                    if (iUpdateSystems == null)
                    {
                        continue;
                    }
                    ...
                    foreach (ILogicUpdateSystem iUpdateSystem in iUpdateSystems)
                    {
                        try
                        {
                            iUpdateSystem.Run(component);
                ...
                {
                    throw new Exception($"entity system logic update fail: {component.GetType().FullName}", e);
                }
        ...
        {
            Queue<EntityRef<Entity>> queue = this.queues[InstanceQueueIndex.LateLogicUpdate];
            int count = queue.Count;
            while (count-- > 0)
            {
                Entity component = queue.Dequeue();
                ...
                if (component is not ILateLogicUpdate)
                {
                    continue;
                }

                List<SystemObject> iLateLogicUpdateSystems = EntitySystemSingleton.Instance.TypeSystems.GetSystems(component.GetType(), typeof (ILateLogicUpdateSystem));
                if (iLateLogicUpdateSystems == null)
                {
                    continue;
                }
                ...
                foreach (ILateLogicUpdateSystem iLateLogicUpdateSystem in iLateLogicUpdateSystems)
                {
                    try
                    {
                        iLateLogicUpdateSystem.Run(component);
                    }
                    catch (Exception e)
                    {
                        Log.Error(e);
