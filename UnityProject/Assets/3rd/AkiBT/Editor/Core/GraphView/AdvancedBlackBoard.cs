    ...
    public class AdvancedBlackBoard : Blackboard, IBlackBoard
    {
        ...
        private void InitRequestDelegate()
        {
            addItemRequested = _blackboard =>
            {
                ...
                menu.AddItem(new GUIContent("SObject"), false, () => AddSharedVariableWithNotify(new SharedSObject()));
                menu.ShowAsContext();
            };
        ...
        public void AddSharedVariable(SharedVariable variable)
        {
            ...
            else if(variable is SharedSObject sharedSObject)
            {
                placeHolder.Add(GetConstraintSObjectField(sharedSObject, (SObjectField)valueField));
            }
        ...
        private VisualElement GetConstraintSObjectField(SharedSObject sharedObject, SObjectField objectField)
        {
            const string NonConstraint = "No Constraint";
            var placeHolder = new VisualElement();
            string constraintTypeName;
            ...
            button.clicked += () =>
            {
                var provider = ScriptableObject.CreateInstance<SObjectTypeSearchWindow>();
                provider.Initialize((type) =>
                {
                    if (type == null)
                    {
